...
"""
Log density gradients wrt theta
"""
function dlogdens( model::lda, indices )
    n_grads = length( indices )
    dlogdens = zeros( n_grads, model.K, model.V )
    exp_theta = exp( model.theta )
    for k in 1:model.K
        for (i,m) in enumerate(indices)
            zcount_theta_ratio = sum( model.zcounts[m,k,:] )/sum( exp_theta[k,:] )
            for v in 1:model.V
                dlogdens[i,k,v] += model.zcounts[m,k,v] - exp_theta[k,v]*zcount_theta_ratio
            end
        end
    end
    return dlogdens
